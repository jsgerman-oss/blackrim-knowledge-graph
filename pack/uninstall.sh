#!/usr/bin/env bash
# blackrim-knowledge-graph — uninstall lifecycle (reverses everything install.sh did).
#
...
#        --town       -> drops  <city>/pack.toml   [imports.blackrim-knowledge-graph]
#        --rig <name> -> drops  <city>/city.toml   [rigs.imports.blackrim-knowledge-graph]
...
PACK_NAME="blackrim-knowledge-graph"
...
uninstall.sh (--town | --rig <name>) [--dry-run] [--city <path>] [--purge] [--no-reload]
  ...
  --town          remove the city-wide import (<city>/pack.toml)
  --rig <name>    remove the rig-scoped import (<city>/city.toml)
  --city <path>   city root (default: walk up from this pack to the nearest city.toml)
  --purge         also delete the engine .venv
  --dry-run       print the plan; change nothing
  --no-reload     skip `gc reload`
  -h, --help      this help
EOF
  exit "${1:-0}"
}
...
[ -n "$CITY" ] && [ -f "$CITY/city.toml" ] || die "no city.toml found above $PACK_DIR (pass --city <path>)"
CITY="$(cd "$CITY" && pwd)"

GC=(gc --city "$CITY")

step "blackrim-knowledge-graph uninstall"
...
step "verify"
if [ "$DRY_RUN" -eq 1 ]; then
  step "blackrim-knowledge-graph uninstall — DRY RUN complete (no changes made)"; exit 0
fi
fail=0
...
echo
if [ "$fail" -eq 0 ]; then
  step "blackrim-knowledge-graph uninstall complete ($SCOPE${RIG:+ $RIG})"
  info "Backups (*.bkg.bak.*) were left in place; remove them when satisfied."
  exit 0
else
  step "blackrim-knowledge-graph uninstall finished WITH WARNINGS"
