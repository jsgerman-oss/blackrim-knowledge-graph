#!/usr/bin/env bash
# blackrim-knowledge-graph — install lifecycle (reversible, idempotent).
#
#   install.sh (--town | --rig <name>) [--dry-run] [--city <path>] [--no-reload]
#
# Turns the blackrim-knowledge-graph pack ON at one of two scopes:
#   --town        city-wide: the pack's bin/kg + skills become available to the
#                 whole city.
...
#      optional — bin/kg also runs under any system python3).
...
#        --town       ->  <city>/pack.toml   [imports.blackrim-knowledge-graph]
#        --rig <name> ->  <city>/city.toml   [rigs.imports.blackrim-knowledge-graph]
#      source is recorded relative to the city root (e.g. the pack subdir
#      "packs/blackrim-knowledge-graph/pack").
#   3. Trigger re-projection with `gc reload` so the pack's bin/skills surface.
#   4. Verify: `gc lint`, the import is registered, and a `kg --version` smoke.
#
# Like cockpit / provider-forge this pack ships NO prompt fragment and NO overlay
# hook — it is a CLI + skills — so the install is: venv + import + reload + verify.
#
...
# and changes nothing. Fails loudly on unexpected state. Reverse with uninstall.sh.
...
PACK_NAME="blackrim-knowledge-graph"   # import binding name
IMPORT_NAME="blackrim-knowledge-graph"
...
install.sh (--town | --rig <name>) [--dry-run] [--city <path>] [--no-reload]
  ...
  --town          install city-wide (writes <city>/pack.toml [imports.blackrim-knowledge-graph])
  --rig <name>    install for one rig   (writes <city>/city.toml [rigs.imports.blackrim-knowledge-graph])
  --city <path>   city root (default: walk up from this pack to the nearest city.toml)
  --dry-run       print the plan; change nothing
  --no-reload     skip `gc reload` (apply on next gc start)
  -h, --help      this help
EOF
  exit "${1:-0}"
}
...
# Locate the city: the nearest ancestor of this pack that holds a city.toml.
# (This pack nests under the repo at packs/<repo>/pack, so walk up rather than
# assuming a fixed depth. Pass --city to override, e.g. when run from the source rig.)
if [ -z "$CITY" ]; then
  d="$PACK_DIR"
  while [ "$d" != "/" ]; do
    if [ -f "$d/city.toml" ]; then CITY="$d"; break; fi
    d="$(dirname "$d")"
  done
fi
[ -n "$CITY" ] && [ -f "$CITY/city.toml" ] || die "no city.toml found above $PACK_DIR (pass --city <path>; vendor this repo under <city>/packs/ first)"
CITY="$(cd "$CITY" && pwd)"

# Source path recorded in the import: relative to the city root if the pack lives
# under it (a bare relative path, e.g. "packs/blackrim-knowledge-graph/pack"),
# else absolute.
...
step "blackrim-knowledge-graph install"
...
backup_file() { # back up $1 once per run to <file>.bkg.bak.<ts>
  local f="$1"
  [ -f "$f" ] || return 0
  local b="${f}.bkg.bak.$(date +%Y%m%d%H%M%S)"
...
if [ "$NO_RELOAD" -eq 1 ]; then
  ...
  info "[dry-run] gc reload  (surfaces the pack's bin/kg + skills)"
else
...
step "4/4  verify"
if [ "$DRY_RUN" -eq 1 ]; then
  info "[dry-run] would verify: gc lint, import registered, kg --version smoke"
  step "blackrim-knowledge-graph install — DRY RUN complete (no changes made)"
  exit 0
fi
...
# kg smoke (best-effort): proves the CLI resolves the engine under the venv.
if [ -x "$PACK_DIR/bin/kg" ]; then
  if "$PACK_DIR/bin/kg" --version >/dev/null 2>&1; then
    info "kg: $("$PACK_DIR/bin/kg" --version)"
  else
    info "kg: --version failed (check setup.sh ran and the engine is importable)"
    fail=1
  fi
fi
...
echo
if [ "$fail" -eq 0 ]; then
  step "blackrim-knowledge-graph install complete ($SCOPE${RIG:+ $RIG})"
  info "Next: 'bin/kg build <path>' to map a project, then 'bin/kg export report'."
  info "Reverse with: $PACK_DIR/uninstall.sh $REVERSE_FLAGS"
  exit 0
else
  step "blackrim-knowledge-graph install finished WITH WARNINGS ($SCOPE${RIG:+ $RIG})"
